...
import { score, average } from './Utils/helperFunctions';
...
function App() {
	...
	const startPage = () => (
		...
		<div className="card">
			<div className="row">
				<h2>How to play:</h2>
				<p>
					The game is not to guess the correct answer but to guess the answer that most people think! We will
					compare your answers to the average from the population.
				</p>
			</div>
			<div className="row">
				<h2>Scoring:</h2>
			...
			<div className="row">
				<button className="start-button" type="submit" onClick={() => setQuestion(1)}>
					{' '}
					Start the quiz!
				</button>
			</div>
	...
	const quizPage = () => {
		...
		const displayResult = () => (
			<div>
				<Answer forward={forward} question={questions[question - 1]} />
			</div>
		);
		return <div>{displayAnswer === false ? displayQuestion() : displayResult()}</div>;
	};
	const endPage = () => {
		...
		return (
			<div className="card">
				...
				<button className="start-button" type="submit" onClick={() => setQuestion(0)}>
					Play again
				</button>
			</div>
		);
	...
	return (
		<div className="box-container">
			...
			{question === 0 ? startPage() : question > 0 && question < questions.length + 1 ? quizPage() : endPage()}
		</div>
	);
}
